...
// Changes by Xzocker: added "leash" (Hypno-Knife) to pillages on this map
main()
{
...
beacon_alien_init_override()
{
	...
	{
		if (isdefined(self.emc) && self.emc == 1 )
...
// added level.pillageinfo.easy/medium/hard_leash to the list to make hypno-knifes spawn on Mayday. The higher the number at the end, the bigger the chance it will spawn
mp_alien_beacon_pillage_init()
{
    ...
    level.pillageinfo.hard_specialammo = 19;
        level.pillageinfo.hard_leash = 5;
...
beacon_door_encounter_logic()
{
    ...
    for (;;)
    {
		...
		{
			if ( var_0 == "door_hive_1" || var_0 == "door_hive_3" )
			{
				level thread maps\mp\alien\_spawn_director::spawnphantoms( 1, 1 );
				level thread maps\mp\alien\_spawn_director::spawnphantoms( 10, 1 );
				if ( !maps\mp\alien\_utility::isplayingsolo() )
					level thread maps\mp\alien\_spawn_director::spawnphantoms( 15, 1 );
				level thread maps\mp\alien\_spawn_director::spawnseeders( 20, 1 );
				wait 15;
				var_2 = var_0 + "_" + randomintrange( 1, 4 );
				maps\mp\alien\_spawn_director::activate_spawn_event( var_2 );
			}
			else if ( var_0 == "door_hive_4" )
			{
				level thread maps\mp\alien\_spawn_director::spawnphantoms( 1, 2 );
				level thread maps\mp\alien\_spawn_director::spawnseeders( 5, 2 );
				level thread maps\mp\alien\_spawn_director::spawnphantoms( 10, 2 );
				if ( !maps\mp\alien\_utility::isplayingsolo() )
					level thread maps\mp\alien\_spawn_director::spawnphantoms( 15, 4 );
				level thread maps\mp\alien\_spawn_director::spawnseeders( 20, 2 );
				wait 5;
				var_0 = "door_hive_1";
				var_2 = var_0 + "_" + randomintrange( 1, 4 );
				maps\mp\alien\_spawn_director::activate_spawn_event( var_2 );
			...
			else if ( var_0 == "door_hive_6" )
			{
				level thread maps\mp\alien\_spawn_director::spawnrhinos( 1, 1 );
				level thread maps\mp\alien\_spawn_director::spawnemc( 5, 2 );
				level thread maps\mp\alien\_spawn_director::spawnrhinos( 10, 2 );
				if ( !maps\mp\alien\_utility::isplayingsolo() )
					level thread maps\mp\alien\_spawn_director::spawnrhinos( 15, 2 );
				level thread maps\mp\alien\_spawn_director::spawnemc( 20, 2 );
				wait 15;
				var_0 = "door_hive_1";
				var_2 = var_0 + "_" + randomintrange( 1, 4 );
				maps\mp\alien\_spawn_director::activate_spawn_event( var_2 );
				level thread maps\mp\alien\_spawn_director::spawnphantoms( 5, 2 );
			}
			else if ( var_0 == "door_hive_7" || var_0 == "door_hive_8" )
			{
				level thread maps\mp\alien\_spawn_director::spawnrhinos( 1, 4 );
				level thread maps\mp\alien\_spawn_director::spawnemc( 5, 4 );
				level thread maps\mp\alien\_spawn_director::spawnphantoms( 10, 4 );
				if ( !maps\mp\alien\_utility::isplayingsolo() )
					level thread maps\mp\alien\_spawn_director::spawnrhinos( 15, 2 );
				level thread maps\mp\alien\_spawn_director::spawnemc( 20, 2 );
				wait 15;
				var_2 = var_0 + "_" + randomintrange( 1, 4 );
				maps\mp\alien\_spawn_director::activate_spawn_event( var_2 );
				level thread maps\mp\alien\_spawn_director::spawnrhinos( 5, 2 );
			}
		}
    }
}
...
spawn_weapon_in_boss_area()
{
    ...
    foreach ( var_2 in var_0 )
    {
        ...
        // if ( var_2.script_noteworthy == "weapon_iw6_alienmicrotar_mp" )
        // {
			// if (var_2.origin == (-1212.9, 2470.5, 158.5))
			// {
				// var_2.origin = (-1218.9, 2470.5, 159);
				// continue;
			// }
        // }
        ...
        {
			if (var_2.origin == (898.4, 2238.4, 1168))
			{
				var_2.script_noteworthy = "weapon_iw6_alienm27_mp";
				var_2.origin = (899.4, 2238.4, 1171);
				continue;
			}
        }
        if ( var_2.script_noteworthy == "weapon_iw6_alienrgm_mp" )
        {
			...
			var_2.origin = var_2.origin - (7, 6, 0);
            continue;
        }

        if ( var_2.script_noteworthy == "weapon_iw6_alieng28_mp" )
        {
            var_2.origin = ( -194.3, 830.9, 156.5 );
            continue;
        }

        if ( var_2.script_noteworthy == "weapon_iw6_alienpanzerfaust3_mp" )
        {
			var_2.script_noteworthy = "weapon_iw6_alienhoneybadger_mp";
            if ( var_2.origin == ( -612.9, -3064.5, 27.4 ) )
            {
                var_2.origin = ( -612.9, -3074.5, 9.2 );
                var_2.angles = ( 344.2, 270, 0 );
            }
        }
    }
