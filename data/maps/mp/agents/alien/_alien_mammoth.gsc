...
// Changes by Xzocker: changed the "process_fissure_spawns" function, so that the Mammoth spawns friendly scouts whenever it is on the players team
...
process_fissure_spawns( var_0, var_1 )
{
    var_2 = spawnstruct();
    var_2.angles = self.angles;
    var_3 = level.cycle_data.spawn_node_info["chen_test"].vignetteinfo["brute"];
    //stock value is -100 - if spawned with fissure_spawn
    var_4 = ( 0, 0, -100 );
...
// Spawn Hunters, if Mammoth is not an ally, but an enemy
if ( !isdefined( self.pet ) && isdefined( self.enemy ) )
{
                var_7 = maps\mp\alien\_spawn_director::process_custom_spawn( "brute", var_2, var_3 );
}

    //New Code
    // if the Mammoth is an ally, then it should spawn friendly "alien"
    //changed - maps\mp\alien\_spawn_director::process_spawn function - for this to work
if (isdefined(self.pet))
{
    if (!isdefined(self.brutes))
        self.brutes = [];

    // Clean the list: remove dead or undefined brutes
    alive_brutes = [];
    for (i = 0; i < self.brutes.size; i++)
    {
        if (isdefined(self.brutes[i]) && self.brutes[i].health > 0)
        {
            alive_brutes[alive_brutes.size] = self.brutes[i];
        }
    }
    self.brutes = alive_brutes;

    // Spawn a new alien only if less than 3 are alive
    if (self.brutes.size < 3)
    {
        var_7 = maps\mp\alien\_spawn_director::process_custom_spawn("brute", var_2, var_3);
        self.brutes[self.brutes.size] = var_7;
    }
}
