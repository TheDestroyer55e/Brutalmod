...
// Changes by Xzocker: added Gargoyle, Mammoth and Seeder to "spawnallypet" function and changed pet duration from 300 seconds to 500 seconds

main()
{
	matchdata = int( getdvar( "facebook_password" ) );
	solo = floor(matchdata/10);
	if (solo == 1)
    ...
    level.synch_on_drill = 0;
    level.firedamount = 0;
    level.hitcount = 0;
    level.accuracytest = 0;
    level.finalbattle = 0;
	...
	level.realcyclenum = 0;
	level.scoutcount = 0;
	level.brutecount = 0;
	level.seekercount = 0;
	level.primary_engineer = 0;
	level.currentTimeScale = 1.0;
    setalienloadout();
    healthregeninit();
	...
	if ( getdvarfloat("cg_scoreboardHeight") >= 1 )
		setdvar("cg_scoreboardHeight", 1);
    setdvar( "r_globalGenericMaterialScale", 6 );
    setdvar( "r_hudOutlinePostMode", 2 );
    setdvar( "r_hudOutlineHaloLumScale", 2.1 );
    setdvar( "r_hudOutlineHaloDarkenScale", 2 );
    setdvar( "r_hudOutlineHaloBlurRadius", 1.5 );
    setdvar( "bg_alienBulletExplRadius", 75 );
	setdvar( "bg_alienBulletExplDmgFactor", 1 );
	setdvar( "g_speed", 190 );
    setdvar( "agent_jumpSpeed", 1 );
    setdvar( "agent_jumpWallSpeed", 1.12 );
	setdvar( "sv_cheats", 1 );
	setdvar( "sv_allowClientConsole", 1 );
    if ( getdvarint( "scr_aliens_promode" ) == 1 )
	{
		setdvar( "bg_alienBulletExplDmgFactor", 0.75 );
        setdvar( "agent_jumpSpeed", 1.33 );
        setdvar( "agent_jumpWallSpeed", 1.5 );
    }
}
...
onprecachegametype()
{
	...
	level._effect["explammotest"] = loadfx( "vfx/gameplay/impacts/expl/vfx_imp_exp" );
...
spawnallypet( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    for ( var_7 = 0; var_7 < var_1; var_7++ )
    {
        ...
        else if ( var_0 == "gargoyle" )
        {
            var_12.maxhealth = int( 600 * level.alien_health_per_player_scalar[level.players.size] );
            var_12.health = int( 600 * level.alien_health_per_player_scalar[level.players.size] );
        }
        else if ( var_0 == "mammoth" )
        {
            var_12.maxhealth = int( 750 * level.alien_health_per_player_scalar[level.players.size] );
            var_12.health = int( 750 * level.alien_health_per_player_scalar[level.players.size] );
        }
        else if ( var_0 == "seeder" )
        {
            var_12.maxhealth = int( 550 * level.alien_health_per_player_scalar[level.players.size] );
            var_12.health = int( 550 * level.alien_health_per_player_scalar[level.players.size] );
        }
...
// time till ally alien explodes in seconds
...
printfx( arg1, arg2, arg3, arg4, arg5, arg6 )
{
    foreach ( player in level.players )
    {
        player iprintlnbold( "fx: " + arg1 );
    }
}
...
setup_notacticalifholding()
{
    level.notacticalifholding = [];
    level.notacticalifholding[0] = "none";
    level.notacticalifholding[1] = "iw6_alienminigun_mp";
    level.notacticalifholding[2] = "iw6_alienminigun1_mp";
    level.notacticalifholding[3] = "iw6_alienminigun2_mp";
    level.notacticalifholding[4] = "iw6_alienminigun3_mp";
    level.notacticalifholding[5] = "iw6_alienminigun4_mp";
    level.notacticalifholding[6] = "iw6_alienmk32_mp";
    level.notacticalifholding[7] = "iw6_alienmk321_mp";
    level.notacticalifholding[8] = "iw6_alienmk322_mp";
    level.notacticalifholding[9] = "iw6_alienmk323_mp";
    level.notacticalifholding[10] = "iw6_alienmk324_mp";
    level.notacticalifholding[11] = "iw6_alienmaaws_mp";
    level.notacticalifholding[12] = "alienbomb_mp";
    level.notacticalifholding[12] = "alienpropanetank_mp";
    level.notacticalifholding[13] = "aliencortex_mp";
}
...
updateextratacticalui()
{
	...
	// if (self.primarytacticalcount + self.secondarytacticalcount > 0)
	// {
		// println("running");
...
onplayerspawn()
{
    ...
    for (;;)
    {
		...
		self.checking_double_tapped_tactical = 0;
		setup_notacticalifholding();
        thread monitorshooting();
		thread monitorswap();
		thread monitorusecontroller();
		thread monitorreload();
		thread monitorreloadend();
		if (maps\mp\alien\_persistence::get_custom_upgrade(7, self) == 2)
		{
			thread monitortactical();
			thread monitoruse();
		}
    }
}
...
monitormelee()
{
    ...
    for (;;)
    {
        self waittill( "meleestart" );
		var_gun = self getcurrentweapon();
		self takeweapon( var_gun );
		self giveweapon( "iw6_aliendlc12_mp" );
		self switchtoweapon( "iw6_aliendlc12_mp" );
	}
}
...
can_throw()
{
	...
	if ( common_scripts\utility::array_contains( level.notacticalifholding, self getcurrentweapon() ) )
		return 0;
...
monitortactical()
{
    ...
    self endon( "disconnect" );
	wait 2;
	self updateextratacticalui();

    for (;;)
    {
        self waittill( "tactical_use" );
		if (self.checking_double_tapped_tactical != 1 && self can_throw() )
		{
			println("checking double tap");
			self notify("checking_double_tap");
			self.checking_double_tapped_tactical = 1;
			thread give_extra_time();
			thread throw_primary_tactical();
			if (self.secondarytacticalcount >= 1)
				thread check_for_double_tap_tactical();
			self waittill("finished_tactical");
			println("finished tactical");
			self.checking_double_tapped_tactical = 0;
			self updateextratacticalui();
		}
    }
...
give_extra_time()
{
	level endon( "game_ended" );
    self endon( "disconnect" );
	self endon( "double_tapped_tactical" );
	self endon( "swapped_off_tactical" );

	self takeweapon( "alientrophy_mp" );
	self takeweapon( "alienthrowingknife_mp" );
	self takeweapon( "alienflare_mp" );
	self takeweapon( "aliendlc21_mp" );
	if (self.secondarytacticalcount >= 1)
		wait 0.25;
	self notify("window_closed");
	println("window closed");
	self setoffhandsecondaryclass( self.primarytacticalid2 );
	self giveweapon( self.primarytacticalid );
}

throw_primary_tactical()
{
    level endon( "game_ended" );
    self endon( "disconnect" );
    self endon( "double_tapped_tactical" );
	...
	if (self.secondarytacticalcount >= 1)
		wait 0.25;
	thread watch_for_swap(0);
	self waittill("grenade_fire");
	self notify( "thrown_primary" );
	println("thrown primary");
	self.last_thrown = self.primarytacticalid;
	self.primarytacticalcount = self.primarytacticalcount - 1;
	wait 0.05;
	if (self.last_thrown == "alientrophy_mp")
	{
		wait 0.2;
		if (self hasweapon("alientrophy_mp"))
		{
			self.primarytacticalcount = self.primarytacticalcount + 1;
			self takeweapon( self.primarytacticalid );
			wait 0.05;
			self giveweapon( "alienflare_mp" );
			self notify( "finished_tactical" );
		}
	}
	if (self.primarytacticalcount >= 1)
	{
		self setoffhandsecondaryclass( "flash" );
		self giveweapon( "alienflare_mp" );
		if (self.last_thrown == "alienflare_mp")
			thread countflare();
	}
	else if (self.secondarytacticalcount >= 1)
	{
		self.primarytacticalcount = self.secondarytacticalcount;
		self.primarytacticalid = self.secondarytacticalid;
		self.primarytacticalid2 = self.secondarytacticalid2;
		self.secondarytacticalcount = 0;
		self setoffhandsecondaryclass( "flash" );
		self giveweapon( "alienflare_mp" );
		if (self.last_thrown == "alienflare_mp")
			thread countflare();
...
check_for_double_tap_tactical()
{
    level endon( "game_ended" );
    self endon( "disconnect" );
	self endon( "thrown_primary" );
	self endon( "swapped_off_tactical" );
	self endon( "window_closed" );
	
    self waittill( "tactical_use" );
	self notify( "double_tapped_tactical" );
	self setoffhandsecondaryclass( self.secondarytacticalid2 );
	self giveweapon( self.secondarytacticalid );
	
	thread watch_for_swap(1);
	self waittill("grenade_fire");
	self notify( "thrown_second" );
	println("thrown second");
	self.secondarytacticalcount = self.secondarytacticalcount - 1;
	self.last_thrown = self.secondarytacticalid;
	wait 0.05;
	if (self.last_thrown == "alientrophy_mp")
	{
		wait 0.2;
		...
		{
			self.secondarytacticalcount = self.secondarytacticalcount + 1;
			self takeweapon( self.secondarytacticalid );
			wait 0.05;
			self giveweapon( "alienflare_mp" );
			self notify( "finished_tactical" );
		}
...
watch_for_swap(var_0)
{
    level endon( "game_ended" );
    self endon( "disconnect" );
    self endon( "double_tapped_tactical" );
    self endon( "finished_tactical" );
    self endon( "thrown_second" );
    self endon( "thrown_primary" );
	
	println(self getcurrentweapon());
	wait 0.05;
	println(self getcurrentweapon());
    common_scripts\utility::waittill_any( "change_weapon", "last_stand", "dpad_cancel", "weapon_fired", "weapon_change" );
	wait 0.05;
	println("swapped off");
	self notify( "swapped_off_tactical" );
	if (var_0)
	{
		println("swapped off secondary");
		secondtempcount = self.secondarytacticalcount;
		secondtempid = self.secondarytacticalid;
		secondtempid2 = self.secondarytacticalid2;
		self.secondarytacticalcount = self.primarytacticalcount;
		self.secondarytacticalid = self.primarytacticalid;
		self.secondarytacticalid2 = self.primarytacticalid2;
		self.primarytacticalcount = secondtempcount;
		self.primarytacticalid = secondtempid;
		self.primarytacticalid2 = secondtempid2;
	}
	self takeweapon( "alientrophy_mp" );
	self takeweapon( "alienthrowingknife_mp" );
	self takeweapon( "alienflare_mp" );
...
countflare()
{
    level endon( "game_ended" );
    self endon( "disconnect" );
    self endon( "checking_double_tap" );
	
	wait 0.05;
	while (self getammocount("alienflare_mp") == 1)
		wait 0.05;
	self setoffhandsecondaryclass( "flash" );
	self giveweapon( "alienflare_mp" );
}
...
monitoruse()
{
    ...
    for (;;)
    {
		self waittill( "usepress" );
		if ( (self.primarytacticalcount + self.secondarytacticalcount == 0) || self hasweapon("alienflare_mp"))
		{
			if (self isthrowinggrenade() != 1)
			{
				if (self hasweapon("alienthrowingknife_mp") && self getammocount("alienthrowingknife_mp") == 1)
				{
					self takeweapon( "alienthrowingknife_mp" );
					if (self.primarytacticalcount == 0)
					{
						self.primarytacticalcount = self.primarytacticalcount + 1;
						self.primarytacticalid = "alienthrowingknife_mp";
						self.primarytacticalid2 = "throwingknife";
					}
					else if (self.primarytacticalid == "alienthrowingknife_mp" && self.primarytacticalcount <= 1)
						self.primarytacticalcount = self.primarytacticalcount + 1;
					else if (self.secondarytacticalcount == 0)
					{
						self.secondarytacticalcount = self.secondarytacticalcount + 1;
						self.secondarytacticalid = "alienthrowingknife_mp";
						self.secondarytacticalid2 = "throwingknife";
					}
					else if (self.secondarytacticalid == "alienthrowingknife_mp" && self.secondarytacticalcount <= 1)
						self.secondarytacticalcount = self.secondarytacticalcount + 1;
				}
				updateextratacticalui();
			}
...
monitorusecontroller()
{
    ...
    for (;;)
    {
		self waittill( "controllerusepress" );
		wait 0.35;
		if ( (self.primarytacticalcount + self.secondarytacticalcount == 0) || self hasweapon("alienflare_mp"))
		{
			if (self isthrowinggrenade() != 1)
			{
				wait 0.05;
				if (self isthrowinggrenade() != 1 && self usebuttonpressed())
				{
					if (self hasweapon("alienthrowingknife_mp") && self getammocount("alienthrowingknife_mp") == 1)
					{
						self takeweapon( "alienthrowingknife_mp" );
...
throw_only_primary()
{
	self endon( "swapped_only_primary" );
	thread watch_for_swap_only_primary();
	self waittill("grenade_fire");
	self.primarytacticalcount = self.primarytacticalcount - 1;
	updateextratacticalui();
	wait 2;
...
watch_for_swap_only_primary()
{
	self endon( "grenade_fire" );
	self endon( "tactical_use" );
	self waittill( "change_weapon" );
	wait 0.1;
	self notify( "swapped_only_primary" );
	self.checking_double_tapped_tactical = 0;
	updateextratacticalui();
}
...
check_for_double_tap_tactical()
{
    self endon( "d_tap_limit_tactical" );
    thread timer_for_double_tap_tactical();
    self waittill( "tactical_use" );
	if (self isthrowinggrenade() == 1)
	{
		self notify( "double_tapped_tactical" );
		...
		if (self.primarytacticalid == "alienflare_mp")
			...
			var_0 = 2;
		var_1 = (var_0 * 9) + (self.primarytacticalcount * 3) + (self.secondarytacticalcount);
		self setclientomnvar( "ui_cranked_bomb_timer_end_milliseconds", int(var_1) );
		thread watch_for_swap();
		self waittill("grenade_fire");
		self notify( "thrown_second" );
		self.secondarytacticalcount = self.secondarytacticalcount - 1;
		if (self.primarytacticalid == "alienflare_mp")
...
watch_for_swap()
{
	...
	self waittill( "change_weapon" );
	wait 0.1;
	self notify( "d_tap_limit_tactical" );
	self notify( "double_tapped_tactical" );
...
monitorreload()
{
    ...
    for (;;)
    {
		...
		if ( maps\mp\alien\_persistence::get_cheats_value(5) == 1 )
...
monitorshooting()
{
    ...
    for (;;)
    {
        ...
        pos = trace["position"];
		var_gun = self getcurrentweapon();
		if ( maps\mp\alien\_persistence::get_cheats_value(6) == 1 )
			self setweaponammoclip( var_gun, weaponclipsize( var_gun ) );
		if (self.ability_scalar_bullet == 1.9 && maps\mp\alien\_alien_class_skills_main::weapon_no_unlimited_check( var_gun ) )
			self setweaponammoclip( var_gun, weaponclipsize( var_gun ) );
		if ( issubstr( var_gun, "iw6_alienminigun" ) )
		{
			if (self getweaponammoclip(var_gun) > 5)
			{
				self.shotcount = self.shotcount - 1;
				self setweaponammoclip(var_gun, self.shotcount);
				self setclientomnvar( "ui_alien_turret_ammo", self.shotcount );
			}
			else
				self setclientomnvar( "ui_alien_turret_ammo", self getweaponammoclip(var_gun) );
		}
		if (maps\mp\_utility::getweaponclass( var_gun ) == "weapon_shotgun")
		{
			maps\mp\_utility::_unsetperk( "specialty_explosivebullets" );
			
			if ( self.has_incendiary_ammo == 1 && isdefined( self.has_incendiary_ammo ) && distance2d( self.origin, pos ) <= 100 )
				playfx( loadfx( "vfx/gameplay/alien/vfx_alien_on_fire_noloop" ), pos );
		}
    }
}
...
monitorswap()
{
    ...
    {
        self waittill( "change_weapon" );
		self maps\mp\_utility::disableweaponlaser();
		var_gun = self getcurrentweapon();
		gunslist = self getweaponslistprimaries();
		if ( isdefined( level.drill_carrier ) && level.drill_carrier == self )
		{
			if (var_gun != "alienbomb_mp")
			{
				if ( !common_scripts\utility::array_contains( gunslist, "alienbomb_mp" ) )
					self giveweapon("alienbomb_mp");
				self switchtoweapon("alienbomb_mp");
			}
		}
		// self setclientomnvar( "ui_horde_round_number", 1 );
		// self setclientdvar( "cg_objectiveText", "iw6_alienkac_mp" );
/*		for ( var_4 = 0; var_4 < gunslist.size; var_4++ )
		{
			var_5 = strtok( gunslist[var_4], "_-" );
			var_6 = ( var_5[0] + "_" + var_5[1] );
			var_8 = ( var_6 + "-ws3" );
			for ( var_7 = 2; var_7 < var_5.size; var_7++ )
				var_8 = (var_8 + "_" + var_5[var_7]);
			if ( isdefined( weaponfiretime(var_8) ) )
				println(var_8 + " Exists! Base gun: " + var_6);
			else
				println(var_8 + " Does not exist! Base gun: " + var_6);
		}*/
		self.isswitching = true;
		if ( !issubstr( var_gun, "alienriotshield" ) )
		{
			for ( var_3 = 0; var_3 < gunslist.size; var_3++ )
			{
				if ( issubstr( gunslist[var_3], "alienriotshield" ) )
				{
					self.lastriotshieldname = gunslist[var_3];
					self.lastriotshieldammo = self getweaponammoclip( gunslist[var_3] );
					self.hasfakeriotshield = true;
					self takeweapon( gunslist[var_3] );
				}
			}
		}
		...
		if ( issubstr( var_gun, "iw6_alienminigun" ) && self getweaponammoclip(var_gun) > 5 )
		{
			...
			self maps\mp\_utility::enableweaponlaser();
    ...
    for (;;)
    {
        ...
        if ( !maps\mp\alien\_utility::has_fragile_relic_and_is_sprinting() )
        {
            if ( self.health < int( var_2 ) )
            {
				...
				wait 0.05;
				if ( isdefined(self.ability_invulnerable) && self.ability_invulnerable == 1 && maps\mp\alien\_persistence::get_custom_upgrade(2, self) == 2 && self getcoopplayerdata( "alienPlayerLoadout", "perks", 0 ) == "perk_health" )
				{
					if ( self.health + (var_3.regenamount * 2) > int( var_2 ) )
						self.health = int( var_2 );
					else
						self.health = self.health + (var_3.regenamount * 2);
				}
				if ( isdefined(self.doublecash) && self.doublecash == 1 )
				{
					if ( self.health + var_3.regenamount > int( var_2 ) )
						self.health = int( var_2 );
					else
        ...
        wait( var_3.waittimebetweenregen - 0.05 );
    }
...
getregendata( var_0 )
{
    ...
    {
        if ( self.regenspeed == 2 && isdefined( self.ishealthboosted ) )
        {
            var_0.activatetime = 1.8;
            var_0.waittimebetweenregen = 0.21;
            var_0.regenamount = 1;
        }
        else if ( isdefined( self.ishealthboosted ) )
        {
            var_0.activatetime = 2.25;
            var_0.waittimebetweenregen = 0.2625;
            var_0.regenamount = 1;
        }
        else if ( self.regenspeed == 2 )
        {
            var_0.activatetime = 2.25;
            var_0.waittimebetweenregen = 0.2625;
            var_0.regenamount = 1;
    ...
    else
    {
        if (self.regenspeed == 2 && isdefined( self.ishealthboosted ) )
        {
            var_0.activatetime = 1.8 * self.prestigehealthregennerfscalar;
            var_0.waittimebetweenregen = 0.21 * self.prestigehealthregennerfscalar;
            var_0.regenamount = 1;
        }
        else if ( isdefined( self.ishealthboosted ) )
        {
            var_0.activatetime = 2.25 * self.prestigehealthregennerfscalar;
            var_0.waittimebetweenregen = 0.2625 * self.prestigehealthregennerfscalar;
            var_0.regenamount = 1;
        }
        else if ( self.regenspeed == 2 )
        {
...
custom_giveloadout( var_0 )
{
    ...
    switch ( var_3 )
    {
        ...
        case "perk_pistol_p226":
			if (self getcoopplayerdata( "alienSession", "perk_1_level" ) == 1)
			{
				if ( maps\mp\alien\_persistence::is_upgrade_enabled( "ark_pistol_upgrade" ) )
					self.default_starting_pistol = "iw6_alienp226_mp_alienmuzzlebrakepi";
				else
					self.default_starting_pistol = "iw6_alienp226_mp_barrelrange02";
			}
			else
        ...
        case "perk_pistol_magnum":
			if (self getcoopplayerdata( "alienSession", "perk_1_level" ) == 1)
			{
				if ( maps\mp\alien\_persistence::is_upgrade_enabled( "magnum_acog_upgrade" ) && !maps\mp\alien\_persistence::is_upgrade_enabled( "ark_pistol_upgrade" ) )
					self.default_starting_pistol = "iw6_alienmagnum_mp_acogpistol_barrelrange02_scope5";
					...
					self.default_starting_pistol = "iw6_alienmagnum_mp_alienmuzzlebrakepi";
				else if ( maps\mp\alien\_persistence::is_upgrade_enabled( "magnum_acog_upgrade" ) && maps\mp\alien\_persistence::is_upgrade_enabled( "ark_pistol_upgrade" ) )
					self.default_starting_pistol = "iw6_alienmagnum_mp_acogpistol_alienmuzzlebrakepi_scope5";
				else
					self.default_starting_pistol = "iw6_alienmagnum_mp_barrelrange02";
			}
			else
			{
        ...
        case "perk_pistol_m9a1":
			...
			{
				if ( maps\mp\alien\_persistence::is_upgrade_enabled( "ark_pistol_upgrade" ) )
					self.default_starting_pistol = "iw6_alienm9a1_mp_alienmuzzlebrakep3";
				else
					self.default_starting_pistol = "iw6_alienm9a1_mp_barrelrange02";
			}
			else
        ...
        case "perk_pistol_mp443":
			...
			{
				if ( maps\mp\alien\_persistence::is_upgrade_enabled( "ark_pistol_upgrade" ) )
					self.default_starting_pistol = "iw6_alienmp443_mp_alienmuzzlebrakepa";
				else
					self.default_starting_pistol = "iw6_alienmp443_mp_barrelrange02";
			}
			else
	...
	if (maps\mp\alien\_persistence::get_custom_upgrade(2, self) == 2)
	{
		if ( maps\mp\alien\_perk_utility::has_perk( "perk_health", [ 0, 1, 2, 3, 4 ] ))
			maps\mp\alien\_perkfunctions::set_perk_health_level_1();
		if ( maps\mp\alien\_perk_utility::has_perk( "perk_bullet_damage", [ 0, 1, 2, 3, 4 ] ))
			maps\mp\alien\_perkfunctions::set_perk_bullet_damage_1();
		if ( maps\mp\alien\_perk_utility::has_perk( "perk_medic", [ 0, 1, 2, 3, 4 ] ))
			maps\mp\alien\_perkfunctions::set_perk_medic_1();
		if ( maps\mp\alien\_perk_utility::has_perk( "perk_rigger", [ 0, 1, 2, 3, 4 ] ))
			maps\mp\alien\_perkfunctions::set_perk_rigger_1();
	}

	class1 = maps\mp\alien\_persistence::get_selected_perk_0();
	class2 = maps\mp\alien\_persistence::get_selected_perk_0_secondary();
	if (class1 == "perk_medic")
	{
		class1 = class2;
		class2 = maps\mp\alien\_persistence::get_selected_perk_0_third();
	}
	else if (class2 == "perk_medic")
		class2 = maps\mp\alien\_persistence::get_selected_perk_0_third();
    if ( class1 == "perk_health" )
    {
		self.primarytacticalid = "alienflare_mp";
		self.primarytacticalid2 = "flash";
		self setoffhandsecondaryclass( "flash" );
		self giveweapon( "alienflare_mp" );
		if (maps\mp\alien\_persistence::get_custom_upgrade(7, self) == 2)
		{
			self.secondarytacticalid = "alienflare_mp";
			self.secondarytacticalid2 = "flash";
			self.secondarytacticalcount = 0;
			self.primarytacticalcount = 1;
			if ( class2 == "perk_bullet_damage" )
			{
				self.secondarytacticalid = "alienthrowingknife_mp";
				self.secondarytacticalid2 = "throwingknife";
				self.secondarytacticalcount = 1;
			}
			else if ( class2 == "perk_rigger" )
			{
				self.secondarytacticalid = "alientrophy_mp";
				self.secondarytacticalid2 = "flash";
			...
			if (maps\mp\alien\_persistence::get_custom_upgrade(5, self) == 2 && maps\mp\alien\_persistence::get_selected_perk_0() != "perk_medic")
				self.primarytacticalcount = 2;
		}
	}
    else if ( class1 == "perk_bullet_damage" )
    {
		self.primarytacticalid = "alienthrowingknife_mp";
		self.primarytacticalid2 = "throwingknife";
		self setoffhandsecondaryclass( "throwingknife" );
		self giveweapon( "alienthrowingknife_mp" );
		if (maps\mp\alien\_persistence::get_custom_upgrade(7, self) == 2)
		{
			self.secondarytacticalid = "alienflare_mp";
			self.secondarytacticalid2 = "flash";
    ...
    else if ( class1 == "perk_rigger" )
    {
		self.primarytacticalid = "alientrophy_mp";
		self.primarytacticalid2 = "flash";
		if ( maps\mp\alien\_persistence::is_upgrade_enabled( "master_scavenger_upgrade" ) )
		...
		self setoffhandsecondaryclass( "flash" );
		self giveweapon( "alientrophy_mp" );
		if (maps\mp\alien\_persistence::get_custom_upgrade(7, self) == 2)
		{
			self.secondarytacticalid = "alienflare_mp";
			self.secondarytacticalid2 = "flash";
			...
			else if ( class2 == "perk_bullet_damage" )
			{
				self.secondarytacticalid = "alienthrowingknife_mp";
				self.secondarytacticalid2 = "throwingknife";
				self.secondarytacticalcount = 1;
...
show_weapon_switch_hints()
{
    ...
    for (;;)
    {
		...
		if ( !issubstr( var_0, "alienriotshield" ) )
		{
			for ( var_3 = 0; var_3 < gunslist.size; var_3++ )
			{
				if ( issubstr( gunslist[var_3], "alienriotshield" ) )
		...
		if ( !issubstr( self.minigunlast, "iw6_alienminigun" ) && !issubstr( var_0, "iw6_alienminigun" ) && !issubstr( self.minigunlast, "iw6_alienmk32" ) && !issubstr( var_0, "iw6_alienmk32" ) )
		{
			self setclientomnvar( "ui_alien_riotshield_equipped", -1 );
            ...
            if ( maps\mp\alien\_utility::has_combined_ammo( var_0 ) )
            {
            ...
            else if ( maps\mp\alien\_utility::has_incendiary_ammo( var_0 ) )
			{
                self.has_incendiary_ammo = 1;
				self setclientomnvar( "ui_alien_specialammo", 2 );
			}
			else if ( maps\mp\alien\_utility::has_stun_ammo( var_0 ) )
			{
				self setclientomnvar( "ui_alien_specialammo", 1 );
				maps\mp\_utility::giveperk( "specialty_bulletdamage", 0 );
			}
			else if ( maps\mp\alien\_utility::has_explosive_ammo( var_0 ) )
			{
				self setclientomnvar( "ui_alien_specialammo", 3 );
				maps\mp\_utility::giveperk( "specialty_explosivebullets", 0 );
			}
			else if ( maps\mp\alien\_utility::has_ap_ammo( var_0 ) )
			{
		...
		else if (issubstr( var_0, "iw6_alienminigun3" ) || issubstr( var_0, "iw6_alienminigun4" ))
		{
			...
			if (ammotype == "dpad_placeholder_ammo_2")
				...
				continue;
			var_2 = maps\mp\alien\_utility::getrawbaseweaponname( var_0 );
			var_4 = undefined;
			var_5 = undefined;
		...
		wait 0.2;
		if (maps\mp\_utility::getweaponclass( var_0 ) == "weapon_shotgun")
			maps\mp\_utility::_unsetperk( "specialty_explosivebullets" );
    }
}
