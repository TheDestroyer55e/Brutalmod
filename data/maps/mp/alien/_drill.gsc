...
drill_think()
{
    ...
    if ( getdvarint( "scr_aliens_promode" ) == 1 )
        level.drill_health_hardcore = 625;
    else
    ...
    {
        if ( getdvarint( "scr_aliens_promode" ) == 1 )
            level.drill_health_hardcore = 1000;
        else
...
set_drill_attack_setup()
{
    ...
    var_0["locust"][0] = maps\mp\alien\_utility::set_attack_sync_direction( ( 0, 1, 0 ), "alien_drill_attack_drill_F_enter", "alien_drill_attack_drill_F_loop", "alien_drill_attack_drill_F_exit", "attack_drill_front", "attack_drill" );
    var_0["locust"][1] = maps\mp\alien\_utility::set_attack_sync_direction( ( -1, 0, 0 ), "alien_drill_attack_drill_R_enter", "alien_drill_attack_drill_R_loop", "alien_drill_attack_drill_R_exit", "attack_drill_right", "attack_drill" );
    var_0["locust"][2] = maps\mp\alien\_utility::set_attack_sync_direction( ( 1, 0, 0 ), "alien_drill_attack_drill_L_enter", "alien_drill_attack_drill_L_loop", "alien_drill_attack_drill_L_exit", "attack_drill_left", "attack_drill" );
    var_1[0] = "offline";
    var_1[1] = "death";
...
watch_to_repair( var_0 )
{
    ...
    for (;;)
    {
        ...
        self.health = level.drill.maxhealth;
        level.drill_last_health = level.drill.maxhealth;
...
set_drill_state_plant( var_0, var_1 )
{
    ...
    level.drill.maxhealth = int( 20000 + var_2 * var_1 maps\mp\alien\_perk_utility::perk_getdrillhealthscalar() );
...
set_drill_state_run( var_0 )
{
	...
	if ( maps\mp\alien\_persistence::get_cheats_value(1) == 1 )
		level.duration_multiplier = 0.01;
	else if ( getdvarint( "scr_aliens_promode" ) >= 1 && level.realcyclenum >= 10 && getdvar( "mapname" ) == "mp_alien_town" )
		level.duration_multiplier = 1.4;
	else if ( getdvarint( "scr_aliens_promode" ) >= 1 && level.realcyclenum >= 11 && getdvar( "mapname" ) == "mp_alien_armory" )
		level.duration_multiplier = 1.1;
	else
		level.duration_multiplier = 1;
    ...
    level.drill.maxhealth = int( 20000 + var_1 * level.drill.owner maps\mp\alien\_perk_utility::perk_getdrillhealthscalar() );
    ...
    self.depth_marker = gettime();
    thread monitor_drill_complete( self.depth*level.duration_multiplier );
    thread maps\mp\alien\_hive::hive_pain_monitor();
    thread maps\mp\alien\_hive::set_hive_icon( "waypoint_alien_defend" );
    destroy_drill_icon();
    maps\mp\alien\_hud::turn_on_drill_meter_hud( self.depth*level.duration_multiplier );
	if ( maps\mp\alien\_persistence::get_cheats_value(1) != 1 )
		level thread watch_drill_depth_for_vo( self.depth*level.duration_multiplier );
}
...
monitor_drill_complete( var_0 )
{
	...
	if ( maps\mp\alien\_persistence::get_cheats_value(1) == 1 )
	{
		level.duration_multiplier = 0.01;
		level.cycle_count++;
		level.oldcyclecount = level.cycle_count;
	}
	else if ( getdvarint( "scr_aliens_promode" ) >= 1 && level.realcyclenum >= 10 && getdvar( "mapname" ) == "mp_alien_town" )
		level.duration_multiplier = 1.4;
	else if ( getdvarint( "scr_aliens_promode" ) >= 1 && level.realcyclenum >= 11 && getdvar( "mapname" ) == "mp_alien_armory" )
    ...
    while ( self.layers.size > 0 )
    {
		...
		var_1_old = var_1;
		var_1 = var_1 * level.duration_multiplier;
        var_2 = self.layers.size == 1;
        var_3 = var_0 - var_1;

        if ( var_2 && maps\mp\alien\_persistence::get_cheats_value(1) != 1 )
        ...
        self.layers = common_scripts\utility::array_remove( self.layers, var_1_old );
        var_0 = var_1;
...
handle_bomb_damage()
{
    ...
    for (;;)
    {
		...
		if (var_6.team == "allies" || maps\mp\alien\_persistence::get_cheats_value(10) == 1 || level.primary_engineer == 1)
        {
            level.drill.health = int(level.drill_last_health);
            continue;
        }
...
useholdthink( var_0, var_1 )
{
    ...
    if ( !var_0 maps\mp\alien\_deployablebox_functions::test_has_perk( "perk_rigger", [ 0, 1, 2, 3, 4 ] ) )
